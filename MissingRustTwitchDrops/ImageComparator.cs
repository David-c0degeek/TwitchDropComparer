...
{
    public List<KeyValuePair<Mat, string>> Compare(List<KeyValuePair<Mat, string>> images1, List<KeyValuePair<Mat, string>> images2)
    {
        return (from image1 in images1 let matchFound = images2.Any(image2 => CompareImages(image1.Key, image2.Key)) where !matchFound select image1).ToList();
    }
